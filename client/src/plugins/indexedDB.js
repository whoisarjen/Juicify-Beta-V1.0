...
Vue.prototype.$getAllIndexedDB = async (value) => {
    let request = await Vue.prototype.$connectIndexedDB();
...
Vue.prototype.$getIndexedDBbyID = async (where, id) => {
    let request3 = await Vue.prototype.$connectIndexedDB()
    return new Promise(resolve => {
...
Vue.prototype.$putIndexedDB = async (where, id, what, value) => {
    await Vue.prototype.$putInformationAboutNeededUpdate(where)
...
Vue.prototype.$putInformationAboutNeededUpdate = async (where) => {
    return new Promise(resolve => {
        (async () => {
            if((where != "nutrition_diary_connections") && (where != "whatToUpdate")){
                if(!store.state.online){
                    let indexedDB = await Vue.prototype.$getIndexedDBbyID("whatToUpdate", where)
                    if(!indexedDB){
                        let array = [{
                            "_id": where,
                        }]
                        await Vue.prototype.$addIndexedDB("whatToUpdate", array)
                    }
